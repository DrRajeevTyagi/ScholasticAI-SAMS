...
import { Teacher, Student, SchoolClass, SubjectPeriodAllocation, User, ClassSession, ExamSchedule, ExamEntry, SchoolEvent, AdmissionField, ExamResult, StudentStream, DisciplinaryAction, CoCurricularActivity, ExamSubjectScore, SchoolContextType, Message, Poll, PollOption, Notice, EventNote } from '../types';
import { STATIC_SCHOOL_DATA } from '../data/staticData';

// Type for imported/exported school data
type ImportedSchoolData = {
    students: Student[];
    classes: SchoolClass[];
    teachers: Teacher[];
    ...
    participatingStudents.forEach(student => {
        ...
        selectedEvents.forEach(ev => {
            ...
            student.activities.push({
                ...
                name: ev.name,
                date: ev.date,
                category: ev.category as CoCurricularActivity['category'],
                type: ev.type,
...
export const SchoolProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const updateEvent = (updatedEvent: SchoolEvent) => {
        // Check if status changed to Completed
        const oldEvent = events.find(e => e.id === updatedEvent.id);
        const justCompleted = oldEvent && oldEvent.status !== 'Completed' && updatedEvent.status === 'Completed';

        setEvents(prev => prev.map(e => e.id === updatedEvent.id ? updatedEvent : e));

        // Auto-sync to activities if just marked complete
        if (justCompleted) {
            // Use setTimeout to ensure state update happens first
            setTimeout(() => syncEventToActivities(updatedEvent.id), 100);
        }
    };
    ...
    // NEW: Event Notes Methods
    const addEventNote = (eventId: string, note: EventNote) => {
        setEvents(prev => prev.map(e =>
            e.id === eventId ? { ...e, notes: [...(e.notes || []), note] } : e
        ));
    };

    const deleteEventNote = (eventId: string, noteId: string) => {
        setEvents(prev => prev.map(e =>
            e.id === eventId ? { ...e, notes: (e.notes || []).filter(n => n.id !== noteId) } : e
        ));
    };

    // NEW: Automatic Event to Activities Sync
    const syncEventToActivities = (eventId: string) => {
        const event = events.find(e => e.id === eventId);
        if (!event || event.status !== 'Completed') return;

        // Get all participants (not volunteers)
        const participants = event.studentRoles.filter(role => role.status === 'participant');

        setStudents(prev => prev.map(student => {
            const participation = participants.find(p => p.studentId === student.id);
            if (!participation) return student;

            // Check if activity already exists
            const activityExists = student.activities?.some(a =>
                a.name === event.name && a.date === event.date
            );
            if (activityExists) return student;

            // Create new activity from event
            const newActivity: CoCurricularActivity = {
                id: `act_${Date.now()}_${student.id}`,
                name: event.name,
                date: event.date,
                category: event.category as CoCurricularActivity['category'],
                type: event.type,
                hoursSpent: event.hoursSpent || 4,
    ...
    return (
        <SchoolContext.Provider value={{
            ...
            addEventNote, deleteEventNote, syncEventToActivities,
